...
layout(binding = 1, std430) buffer ssbo_normal {
    VertexData auxi[];
};
...
void setNormal(int index, vec3 pos) {
    VertexData postruct = VertexData(float[3](pos.x,pos.y,pos.z));
    auxi[index] = postruct;
}
...
*/
    setNormal(int(pos0.w), getNormal(int(pos0.w))+flatnormal);
    setNormal(int(pos1.w), getNormal(int(pos1.w))+flatnormal);
    setNormal(int(pos2.w), getNormal(int(pos2.w))+flatnormal);
